"use client";

import React, { useState } from "react";
import {
  Bed as BedIcon,
  CheckCircle2,
  Clock,
  Sparkles,
  UserCheck,
  Building,
  Filter,
  ArrowRightLeft,
  DollarSign,
} from "lucide-react";
import { MOCK_BEDS, MOCK_PATIENTS } from "@/lib/mock-data";
import { Bed } from "@/types";
import { formatCurrencyBDT } from "@/lib/utils";

export default function BedManagementPage() {
  const [beds, setBeds] = useState<Bed[]>(MOCK_BEDS);
  const [selectedWard, setSelectedWard] = useState("all");
  const [activeBedModal, setActiveBedModal] = useState<Bed | null>(null);

  // Status Metrics
  const totalBeds = beds.length;
  const availableBeds = beds.filter((b) => b.status === "available").length;
  const occupiedBeds = beds.filter((b) => b.status === "occupied").length;
  const cleaningBeds = beds.filter((b) => b.status === "cleaning").length;
  const reservedBeds = beds.filter((b) => b.status === "reserved").length;

  const wardsList = Array.from(new Set(beds.map((b) => b.ward_name)));

  const filteredBeds = beds.filter(
    (b) => selectedWard === "all" || b.ward_name === selectedWard
  );

  const handleUpdateStatus = (
    bedId: string,
    newStatus: "available" | "occupied" | "cleaning" | "reserved",
    patientName?: string
  ) => {
    setBeds((prev) =>
      prev.map((b) =>
        b.id === bedId
          ? {
              ...b,
              status: newStatus,
              patient_name: newStatus === "occupied" ? (patientName || "Admitted Patient") : undefined,
              admitted_at: newStatus === "occupied" ? new Date().toISOString().split("T")[0] : undefined,
            }
          : b
      )
    );
    setActiveBedModal(null);
  };

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <span className="text-xs font-bold text-sky-600 uppercase tracking-wider">
            Inpatient Capacity & Admission Matrix
          </span>
          <h1 className="text-2xl font-black text-slate-900 mt-1">
            Ward & Cabin Bed Management
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Realtime occupancy tracking across General Wards, VIP Cabins, ICU, and CCU with auto-charge generation.
          </p>
        </div>
      </div>

      {/* 1. CAPACITY METRICS STRIP */}
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-2xs">
          <span className="text-[11px] font-bold text-slate-500 uppercase">Total Capacity</span>
          <div className="text-2xl font-black text-slate-900 mt-1">{totalBeds} Beds</div>
          <span className="text-[10px] text-slate-400">All Wards & Cabins</span>
        </div>

        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 shadow-2xs">
          <span className="text-[11px] font-bold text-emerald-800 uppercase">Available</span>
          <div className="text-2xl font-black text-emerald-700 mt-1">{availableBeds} Vacant</div>
          <span className="text-[10px] text-emerald-700 font-medium">Ready for admission</span>
        </div>

        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 shadow-2xs">
          <span className="text-[11px] font-bold text-rose-800 uppercase">Occupied</span>
          <div className="text-2xl font-black text-rose-700 mt-1">{occupiedBeds} Inpatients</div>
          <span className="text-[10px] text-rose-700 font-medium">Active admissions</span>
        </div>

        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 shadow-2xs">
          <span className="text-[11px] font-bold text-amber-800 uppercase">Cleaning / Sanitize</span>
          <div className="text-2xl font-black text-amber-700 mt-1">{cleaningBeds} Beds</div>
          <span className="text-[10px] text-amber-700 font-medium">Housekeeping active</span>
        </div>

        <div className="p-4 rounded-xl bg-sky-50 border border-sky-200 shadow-2xs">
          <span className="text-[11px] font-bold text-sky-800 uppercase">Reserved</span>
          <div className="text-2xl font-black text-sky-700 mt-1">{reservedBeds} Beds</div>
          <span className="text-[10px] text-sky-700 font-medium">Booked for surgery</span>
        </div>
      </div>

      {/* 2. WARD FILTER BAR */}
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs flex flex-wrap gap-2 items-center">
        <span className="text-xs font-bold text-slate-500 uppercase mr-2 flex items-center">
          <Filter className="w-3.5 h-3.5 mr-1" />
          Ward Filter:
        </span>
        <button
          onClick={() => setSelectedWard("all")}
          className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
            selectedWard === "all"
              ? "bg-slate-900 text-white"
              : "bg-slate-100 text-slate-700 hover:bg-slate-200"
          }`}
        >
          All Wards ({totalBeds})
        </button>
        {wardsList.map((ward) => (
          <button
            key={ward}
            onClick={() => setSelectedWard(ward)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
              selectedWard === ward
                ? "bg-sky-600 text-white"
                : "bg-slate-100 text-slate-700 hover:bg-slate-200"
            }`}
          >
            {ward}
          </button>
        ))}
      </div>

      {/* 3. VISUAL BED MATRIX GRID */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {filteredBeds.map((bed) => {
          const isAvailable = bed.status === "available";
          const isOccupied = bed.status === "occupied";
          const isCleaning = bed.status === "cleaning";
          const isReserved = bed.status === "reserved";

          return (
            <div
              key={bed.id}
              onClick={() => setActiveBedModal(bed)}
              className={`p-5 rounded-2xl border cursor-pointer transition shadow-2xs hover:shadow-md flex flex-col justify-between ${
                isAvailable
                  ? "bg-white border-emerald-300 ring-2 ring-emerald-500/10 hover:border-emerald-500"
                  : isOccupied
                  ? "bg-rose-50/50 border-rose-300 hover:border-rose-500"
                  : isCleaning
                  ? "bg-amber-50/50 border-amber-300 hover:border-amber-500"
                  : "bg-sky-50/50 border-sky-300 hover:border-sky-500"
              }`}
            >
              <div>
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">
                    {bed.ward_name}
                  </span>
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${
                      isAvailable
                        ? "bg-emerald-100 text-emerald-800"
                        : isOccupied
                        ? "bg-rose-100 text-rose-800"
                        : isCleaning
                        ? "bg-amber-100 text-amber-800"
                        : "bg-sky-100 text-sky-800"
                    }`}
                  >
                    {bed.status}
                  </span>
                </div>

                <div className="flex items-center space-x-2 my-2">
                  <BedIcon
                    className={`w-6 h-6 ${
                      isAvailable
                        ? "text-emerald-600"
                        : isOccupied
                        ? "text-rose-600"
                        : isCleaning
                        ? "text-amber-600"
                        : "text-sky-600"
                    }`}
                  />
                  <h3 className="font-extrabold text-base text-slate-900 font-mono">
                    {bed.bed_number}
                  </h3>
                </div>

                <div className="text-xs space-y-1 mt-2">
                  <p className="text-slate-600">
                    <strong>Type:</strong> {bed.bed_type}
                  </p>
                  <p className="text-slate-600">
                    <strong>Tariff:</strong> {formatCurrencyBDT(bed.daily_rate)} / day
                  </p>
                </div>
              </div>

              <div className="pt-3 border-t border-slate-200/80 mt-3 text-xs">
                {isOccupied ? (
                  <div>
                    <span className="font-bold text-slate-900 block truncate">
                      {bed.patient_name}
                    </span>
                    <span className="text-[10px] text-slate-500 block">
                      Admitted: {bed.admitted_at}
                    </span>
                  </div>
                ) : isCleaning ? (
                  <span className="text-[11px] text-amber-700 font-medium">
                    Sanitization in progress
                  </span>
                ) : (
                  <span className="text-[11px] text-emerald-700 font-semibold">
                    Click to Allocate Patient →
                  </span>
                )}
              </div>
            </div>
          );
        })}
      </div>

      {/* BED STATUS MODAL / ACTION DIALOG */}
      {activeBedModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 text-xs">
            <h3 className="text-base font-black text-slate-900 mb-1">
              Bed Controller: {activeBedModal.bed_number} ({activeBedModal.ward_name})
            </h3>
            <p className="text-slate-500 mb-4">
              Daily Tariff: {formatCurrencyBDT(activeBedModal.daily_rate)} • Current Status:{" "}
              <strong className="uppercase">{activeBedModal.status}</strong>
            </p>

            <div className="space-y-2">
              <button
                onClick={() =>
                  handleUpdateStatus(
                    activeBedModal.id,
                    "occupied",
                    prompt("Enter patient name to allocate:") || "Admitted Patient"
                  )
                }
                className="w-full text-left p-3 rounded-xl border border-rose-200 bg-rose-50 hover:bg-rose-100 font-bold text-rose-900 flex justify-between items-center transition"
              >
                <span>Allocate Patient (Admit)</span>
                <UserCheck className="w-4 h-4" />
              </button>

              <button
                onClick={() => handleUpdateStatus(activeBedModal.id, "cleaning")}
                className="w-full text-left p-3 rounded-xl border border-amber-200 bg-amber-50 hover:bg-amber-100 font-bold text-amber-900 flex justify-between items-center transition"
              >
                <span>Discharge Patient & Mark for Cleaning</span>
                <Sparkles className="w-4 h-4" />
              </button>

              <button
                onClick={() => handleUpdateStatus(activeBedModal.id, "available")}
                className="w-full text-left p-3 rounded-xl border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 font-bold text-emerald-900 flex justify-between items-center transition"
              >
                <span>Mark Clean & Available</span>
                <CheckCircle2 className="w-4 h-4" />
              </button>
            </div>

            <div className="mt-5 pt-3 border-t border-slate-100 text-right">
              <button
                onClick={() => setActiveBedModal(null)}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-lg"
              >
                Close
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
