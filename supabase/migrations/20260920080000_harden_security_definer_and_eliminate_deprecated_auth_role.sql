-- =====================================================================================
-- Migration 45: Harden SECURITY DEFINER routines, eliminate deprecated auth.role(),
-- suppress SQLERRM leakage, and enforce zero-dependency JWT claims inspection.
-- =====================================================================================

-- 1. Redefine get_current_org_id() without deprecated auth.role()
...
-- 2. Redefine verify_and_record_online_payment() with safe JWT role inspection and sanitized error responses (no SQLERRM leakage)
...
BEGIN
    ...
    -- Resolve caller role safely without deprecated auth.role()
    v_caller_role := COALESCE(
        NULLIF(pg_catalog.current_setting('request.jwt.claims', true)::jsonb->>'role', ''),
        NULLIF(pg_catalog.current_setting('request.jwt.claim.role', true), '')
    );
...
EXCEPTION WHEN OTHERS THEN
    -- Suppress raw SQLERRM leakage to client/caller
    RETURN jsonb_build_object(
        'success', false,
        'code', 'SETTLEMENT_INTERNAL_ERROR',
        'error', 'Payment settlement could not be completed due to an internal processing error.'
    );
END;
$$;
