...
describe("OHMS Phase 34: Billing Pre-Validation Atomicity & Idempotency Hardening", () => {
  ...
  test("5. Desktop download page provides verified .exe and .msi links with actual sizes", () => {
    ...
    assert.match(content, /Onnesha-Hospital-Setup-1\.0\.0\.exe/, "Must link to NSIS setup executable");
    assert.match(content, /Onnesha-Hospital-1\.0\.0\.msi/, "Must link to MSI package");
  ...
  test("6. Desktop latest.json manifest points to valid deployment assets", () => {
    ...
    assert.match(manifest.platforms["windows-x86_64"].installer_exe, /Onnesha-Hospital-Setup-1\.0\.1\.exe/);
    assert.match(manifest.platforms["windows-x86_64"].installer_msi, /Onnesha-Hospital-1\.0\.1\.msi/);
